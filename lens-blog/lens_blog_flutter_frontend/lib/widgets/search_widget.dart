...
import 'package:lens_blog_flutter_frontend/logic/query_logic.dart';

class SearchWidget extends StatefulWidget {
  final Map dataMap;
  final List<ArticleItem> itemList;
  const SearchWidget({Key key, @required this.dataMap, @required this.itemList})
      : super(key: key);
...
class _SearchWidgetState extends State<SearchWidget> {
  ...
  int getArticleItemIdByName(String name) {
    widget.itemList.forEach((element) {
      if (element.title == name) return element.id;
    });
    return -1;
  }
}
